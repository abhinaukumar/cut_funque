...
def ngan_spat(d2h=1.5, k=21, channel=0):
    if d2h == 1.5 and k == 21:
...
def nadenau_spat(d2h=1.5, k=5, channel=0):
    lookup_lens = [5, 7, 5]
    if d2h == 1.5 and k == lookup_lens[channel]:
...
def watson(level, subband, channel=0, pic_height=2160):
    '''
    level: 0 indexed level of the discrete wavelet transform
    subband: 0 indexed in the list [approximation, horizontal, vertical, diagonal]
    Ref: A. Watson, G. Yang, et al. "Visibility of Wavelet Quantization Noise"
    '''
    if level <= 4 and pic_height == 2160:
...
def hill(level, subband, channel=0, pic_height=2160):
    '''
    level: 0 indexed level of the discrete wavelet transform
    subband: 0 indexed in the list [approximation, horizontal, vertical, diagonal]
