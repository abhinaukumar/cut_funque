...
def filter_pyr(pyr, csf_funct, channel=0):
    ...
    filt_details = []
    filt_approxs = []
    for lev, (approx_level, detail_level) in enumerate(zip(approxs, details)):
        filt_level = []
        ...
        if csf_funct.__name__ != 'ahc_weight':
            filt_approxs.append(approx_level * csf_funct(lev, 0, channel=channel))
        else:
            filt_approxs.append(approx_level * csf_funct(lev, 0, n_levels, channel=channel))

        filt_details.append(tuple(filt_level))
    return filt_approxs, filt_details
...
def filter_img(img, filter_key, wavelet=None, channel=0, **kwargs):
    if filter_key is None:
        ...
        n_levels = kwargs.pop('levels', 4)
        pyr = wavedec2(img, wavelet, 'reflect', n_levels)
        details = pyr[1:]
        approxs = [0]*(len(details)-1) + pyr[:1]
        csf_funct = csf_dict[filter_key]

        pyr_filtered = filter_pyr((approxs, details), csf_funct, channel=channel)
        pyr_filtered = approxs[-1:] + pyr_filtered[1]
